...
local parse_bag = function(id, num)
	...
	for i = 1, num do
		...
		if item_link then
			...
			bag_contents[i].item_id = item_id
			_,bag_contents[i].count = GetContainerItemInfo(id, i)
			-- detailed info
			bag_contents[i].id,
...
local update_bank = function()
	...
	for bag_on = 1, GetNumBankSlots() do
		...
		if link then
			...
			-- get the texture
			local item_id = GetInventoryItemTexture("player",(ContainerIDToInventoryID(bag)))
			-- save the bag info
			GUILDLAUNCH_SVARS.bank["Bag"..bag_on] = {item_name = item_name, bag_id = id, item_id = item_id, contents = parse_bag(bag, GetContainerNumSlots(bag)) }
		end
	end
...
local update_inventory = function()
	...
	for bag = 1, NUM_BAG_FRAMES do
		...
		if link then
			local _, _, id = string.find(link, "item:(%d+):")
			GUILDLAUNCH_SVARS.inventory[name] = { bag_id = id, item_id = item_id, contents = parse_bag(bag, GetContainerNumSlots(bag)) }
		end
	end
end
...
--[[local update_active_pet = function()
	...
	petIcon, petName, petLevel, petFamily, petTalent = GetStablePetInfo(0)
	DEFAULT_CHAT_FRAME:AddMessage("|cffff8c00--"..petName.."<---|r")
	if petName then
		GUILDLAUNCH_SVARS.pets[petName] = {}
		GUILDLAUNCH_SVARS.pets[petName] = {texture = petIcon, level = petLevel, family = petFamily, talent = petTalent}
		-- DEFAULT_CHAT_FRAME:AddMessage("|cffff8c00--"..petName.."|r")
	end
end--]]


local update_pet_info = function()
	-- get the pet info
	GUILDLAUNCH_SVARS.pets = GUILDLAUNCH_SVARS.pets or {}
	local num_stable_pets = NUM_PET_STABLE_SLOTS
	-- 0 is the first pet
	petIcon, petName, petLevel, petFamily, petTalent = GetStablePetInfo(0)
	...
	for i=1, num_stable_pets do
		petIcon, petName, petLevel, petFamily, petTalent = GetStablePetInfo(i)
		if petName then
			GUILDLAUNCH_SVARS.pets[petName] = {}
			GUILDLAUNCH_SVARS.pets[petName] = {texture = petIcon, level = petLevel, family = petFamily, talent = petTalent}
		end
...
local onevent = function(self, event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9)
	...
	if event == "PLAYER_LOGIN" then
		local mod_version = GetAddOnMetadata("ClassicGuildLaunchProfiler", "Version")
		local export_version = GetAddOnMetadata("ClassicGuildLaunchProfiler", "X-ExportVersion")
