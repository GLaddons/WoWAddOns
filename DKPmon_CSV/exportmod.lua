...
function Export:GetFubarItem()
   ...
   self.fubaritem = { 
      type = 'execute',
      name = L['Points database CSV export'],
...
function Export:PerformExport()
   ...
   for name, dbtab in pairs(pointsdb) do
	 ...
	 cstrtab[4+pool] = tostring(dkpsys:GetPlayerPoints(charInfo.name, pool))
      end
...
DKPmon.ImpExpModules:RegisterExportModule(Export, "CSV")